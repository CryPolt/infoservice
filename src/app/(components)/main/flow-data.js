...
const dataFilePath = path.join(process.cwd(), 'public', './flow-data.json'); // Path to your file

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        try {
            const flowData = req.body; // Assuming flow data is sent as JSON in the request body
            fs.writeFileSync(dataFilePath, JSON.stringify(flowData)); // Write to the file
